...
create table ezd_role( --   角色表
	role_id int primary key  AUTO_INCREMENT,
	role_name varchar(20) not null
);
create table ezd_permission( --   权限表
	per_id int primary key ,
	per_name varchar(20) not null
);
create table ezd_tole_permission(--   角色权限关系表
	roper_id int primary key auto_increment,
	roper_role int references ezd_role(role_id), --  对应的那个权限
	reper_perm  int references ezd_permission(per_id)	--  那个角色对应了权限
);
...
create table ezd_role_user(--  用户角色关系表
	...
	uper_role int references ezd_role(role_id), --  对应的那个角色
	uper_umg int references ezd_umg(umg_id) --  那个用户对应的角色
);
...
create table ezd_enmg(--  企业信息表
	...
	enmg_type int  references ezd_entype(entype_id),--  企业类型	
...
create table ezd_schmg(--  学校信息表
	...
	schmg_type int references ezd_schtype(schtype_id),--  学校性质 
...
create table ezd_uminden(--  关注记录表
	...
	uminden_user  int references ezd_umg(umg_id),--  谁关注的 对应用户信息
	uminden_en int references ezd_enmg(enmg_id), --  关注了谁 对应公司信息表	
...
create table ezd_jobmg(--  用户兼职记录信息表
	...
	jobmg_user int  REFERENCES ezd_umg(umg_id),
	jobmg_num int ,--  包名次数
...
create table ezd_unews(--  用户消息表
	...
	unews_user int references ezd_umg(umg_id),--   受信人--  用户信息
...
create table ezd_enret(--  招聘信息表
	...
	nret_enmg int references ezd_enmg(enmg_id),
	enret_wantNum int  not null,--  招聘人数
	enret_type int references ezd_retType(retType_id),--   招聘类型 （实习 兼职 全职）
	...
	enret_status int references ezd_newsStatus(newsStatus_id)  --  信息状态,( 进行中  已结束)	
);
create table ezd_erlenret(--  招聘信息报名表
	erlenret_id int primary key auto_increment,
	erlenret_umg int references umg(umg_id), --  谁报名了
	erlenret_enret int references enret(enret_id), --  报名的是那个招聘信息
	erlenret_status int references ezd_restatus(restatus_id) --  报名状态 (已报名 审核 录用 拒绝)
);
create table ezd_enret_browse(--  浏览记录表
	browse_id int primary key auto_increment,
	browse_user int references ezd_umg(umg_id),--   谁浏览的 对应用户信息
	browse_en int references ezd_enret(enret_id)--   浏览的信息 对应 招聘信息
);
create table ezd_bigret(--  大招会信息表
	...
	bigret_status int references ezd_newsStatus(newsStatus_id),--  大招会状态(未开始  进行中 已结束)
	bigret_creTime timestamp default current_timestamp,--  发布时间
	bigret_detail text  not null,--  大招会详情
	bigret_post varchar(20),--  招聘职位
	bigret_enmg int references ezd_enmg(enmg_id)--   哪家公司发布的
);
create table ezd_erlbigret(--  大招会报名表
	erlbigret_id int primary key auto_increment,
	erlbigret_mes int references ezd_bigret(bigret_id), --  报名了那个大招会
	erlbigret_user int references ezd_umg(umg_id), --   谁报名了大招会
	erlbigret_status int references ezd_restatus(restatus_id), --  报名状态 (已报名 审核 录用 拒绝)
	erlbigret_time timestamp default current_timestamp
);
create table ezd_schret(--  校招会
	schret_id int primary key auto_increment,
	schret_schmg int references ezd_schmg(schmg_id),--   那所学校开校招会
	schret_enmg int references ezd_enmg(enmg_id),--  那家公司在招聘
	schret_status int references ezd_newsStatus(newsStatus_id),--  校招聘状态 （未开始 已结束 进行中)
...
create table ezd_erlschret(--  校招会包名表
	...
	erlschret_user int references ezd_umg(umg_id),--   谁报名了
	erlschret_schret int references ezd_schret(schret_id),--   报名了那个校招会\
	erlschret_status int references ezd_restatus(restatus_id), --  包名状态 (已报名 审核 录用 拒绝)
...
create table ezd_article(--  文章信息表
	...
	article_type int references ezd_artitype(artitype_id),--  文章类型
...
create table ezd_readarti(
	readarti_id int primary key auto_increment,
	readarti_article int references ezd_article(article_id), -- 阅读了那篇文章
	readarti_user int references ezd_umg(umg_id),-- 谁阅读了
	readarti_time timestamp default current_timestamp
);
create table ezd_sayart(--  文章评论
	sayart_id int primary key auto_increment,
	sayart_article int references ezd_article(article_id),--  评论的文章
	sayart_umg int references ezd_umg(umg_id),--   评论人
...
create table ezd_point(--  点赞信息表
	...
	point_article int references ezd_article(article_id),--  点赞的文章
	point_umg int references ezd_umg(umg_id),--   点赞人	
